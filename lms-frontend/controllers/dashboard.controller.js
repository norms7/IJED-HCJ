...
const DashboardController = {
  ...
  async _postRender(sectionId) {
    ...
    if (sectionId === 'dashboard' && role === 'admin') {
      ...
      try {
        ...
        area.innerHTML = AdminView.dashboard(user, {
          total_users: 0, total_teachers: 0, total_students: 0,
          total_modules: 0, total_activities: 0, recent_users: [],
        });
      } finally {
        Loader.done();
      }
    ...
    if (sectionId === 'modules' && role === 'teacher') {
      ...
      try {
        const [subjects, modules] = await Promise.all([
          api.getMySubjects(), api.getMyModules(),
        ]);
        const subjectMap = {};
    ...
    if (sectionId === 'modules' && role === 'student') {
      ...
      try {
        const [subjects, modules] = await Promise.all([
          api.getStudentSubjects(), api.getStudentModules(filterSubjectId),
        ]);
        const subjectMap = {};
        subjects.forEach(s => { subjectMap[s.subject_id] = s.subject_name; });
        modules.forEach(m => { m._subject_name = subjectMap[m.subject_id] || 'Unknown'; });
    ...
    if (sectionId === 'activities' && role === 'teacher') {
      ...
      try {
        const [activities, subjects] = await Promise.all([
          api.getTeacherActivities(), api.getMySubjects(),
        ]);
        const subjectMap = {};
        subjects.forEach(s => { subjectMap[s.subject_id] = s.subject_name; });
    ...
    if (sectionId === 'manage-users') {
      ...
      Loader.init();
      try {
        const [usersRes, teachersRes, studentsRes, sectionsRes] = await Promise.all([
