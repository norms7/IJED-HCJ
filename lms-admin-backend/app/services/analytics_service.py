...
async def get_grade_progress(
    ...
    subjects_in_data = []
    for d in data:
        if d["subject_id"] not in seen:
            seen.add(d["subject_id"])
            subjects_in_data.append({"subject_id": d["subject_id"]})
...
async def get_attendance_calendar(
    ...
    # Fetch all sessions (has_class=True and False) for this student's subjects
    ...
    for sess in sessions:
        ...
        # Last-write wins if multiple sessions on same date (multi-subject day)
        calendar[key] = status
...
async def get_score_vs_class_average(
    ...
    # Get activities in these subjects
    ...
    # Get all graded submissions for these activities
    ...
    # Aggregate: per activity → list of pct scores, student's pct
...
async def get_subject_radar(
    ...
    # Fetch graded submissions, join activity for subject_id
    subs_result = await db.execute(
        select(ActivitySubmission)
...
async def get_predicted_final_grade(
    ...
    # ── Prior ────────────────────────────────────────────────────────────────
    MU_0    = 78.0   # school historical mean grade %
    TAU_0   = 1.0 / (12.0 ** 2)   # prior precision (1/variance)
    SIGMA_L = 15.0   # assumed likelihood noise (grade spread)
    TAU_L   = 1.0 / (SIGMA_L ** 2)
...
async def get_improvement_probability(
    ...
    # Posterior mean P(success on next activity)
...
async def get_students_like_you(
    ...
    async def _student_profile(sid: int) -> dict:
        # Attendance rate
        att_sessions = await db.execute(
            select(AttendanceSession)
            .where(
        ...
        sessions = att_sessions.scalars().all()
        ...
        if session_ids:
            ...
            present = len(rec_res.scalars().all())
            att_rate = present / len(session_ids) * 100
        ...
        total_mods = mods_res.scalar() or 0
        reads_res = await db.execute(
            select(func.count()).where(StudentModuleRead.student_id == sid)
        )
        reads = reads_res.scalar() or 0
        mod_rate = (reads / total_mods * 100) if total_mods else 0
        ...
        subs = [s for s in subs_res.scalars().all()
                ...
                and s.max_score and s.max_score > 0]
        avg_score = (
            sum((s.score / s.max_score) * 100 for s in subs) / len(subs)
            if subs else 0
        )

        return {"att_rate": att_rate, "mod_rate": mod_rate, "avg_score": avg_score}

    # ── Build anonymised peer profiles ────────────────────────────────────────
    # Get all other students in the same subjects
    enroll_res = await db.execute(
        select(StudentSubjectEnrollment.student_id)
        .where(StudentSubjectEnrollment.subject_id.in_(subject_ids))
    )
    peer_ids = [r[0] for r in enroll_res.all() if r[0] != student_id]

    me = await _student_profile(student_id)

    # Composite engagement score = mean of three normalised dimensions
    def _composite(p: dict) -> float:
        return (p["att_rate"] + p["mod_rate"] + p["avg_score"]) / 3

    my_composite = _composite(me)

    # Sample up to 200 peers (avoid expensive full-class scan on large schools)
    sampled_peers = peer_ids[:200]
    peer_composites: list[float] = []
    for pid in sampled_peers:
        try:
            p = await _student_profile(pid)
            peer_composites.append(_composite(p))
        except Exception:
            continue
    ...
    return {
        ...
        "message": message,
        "my_profile": {
            "attendance_rate": round(me["att_rate"], 1),
            "module_completion": round(me["mod_rate"], 1),
            "avg_score": round(me["avg_score"], 1),
        },
...
async def get_risk_assessment(
    ...
    # ── Logistic model ────────────────────────────────────────────────────────
    # Weights learned from typical educational risk factors
    # Higher values → higher P(fail)
    #  att_rate:      -3.5  (low attendance → higher risk)
    #  mod_rate:      -1.5
    #  act_completion:-2.0
    #  avg_score:     -4.0  (low score most predictive)
    # Intercept chosen so that a student at 75% across all → P(fail) ≈ 0.15
